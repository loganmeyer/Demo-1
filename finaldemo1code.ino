...
#define voltageLDir       5   // Direction for Right motor
#define pinD2             4  
...
float  goalDistance = 7;            // Desired linear distance traveled, in ft (CHANGE PER RUN)
...
float goalAngleAngle = (goalAngle*(float) wheelbase/ (float) diametermodspin ) - (float) 0.08; //wheelbas*wheelbase/r
...
void setup() {   
  ...
  KiL = KiL - ((goalDistance/((float) 10))* (float) 0.00015);      // Ki is proportional to the requested distance to prevent abrupt stopping
  KiR = KiR - ((goalDistance/((float) 10))* (float) 0.00015);      // Longer distance --> lower Ki since there is available time to spool up/down slowly
...
void loop() {
  ...
  if((triggertime > 0) && (millis() > (triggertime+timetogo))){
    if(counter == 0){
      ...
      goalDistanceAngle = (float) 2.1/ (float) diametermod;
      ifspin = 0;
      angularPositionR = 0;
      angularPositionL = 0;
      //directiongoing = 1;
...
Serial.println(angularPositionR/((float) wheelbase/ (float) diametermodspin));
